...
$BoolP3 = Test-Path "$wFolder\# ZIM Files"
if ($BoolP3 -eq $false)
{
...
# Append commands to text file in current directory then runs them
    ...
    $dockerCMD = "run -v $wFolder\$itemname2`:/crawls docker.io/webrecorder/browsertrix-crawler:1.3.5 crawl --name $itemname2 --url $item --scopeType $scopeType1 --depth -1 --extrahops 1 --timeout 10000 --waitUntil load,networkidle2 --postLoadDelay 2 --pageExtraDelay 1 --workers 6 --exclude=`"(action=edit|Special:UserProfileActivity|Special:ListFiles|Special:AbuseLog|Special:Log|User_blog|action=|\?diff|youtube.com|silver_surfer.html|index.php\?title|redirect=)`" --keep --verbose --continue-on-error"
...
Start-Process -NoNewWindow -FilePath $dockerPath -ArgumentList "run -v $wFolder\$item`:/output -v $inputPath`:/input ghcr.io/openzim/zimit:2.1.4 zimit --warcs $WARCs --name $item --keep --continue-on-error"
