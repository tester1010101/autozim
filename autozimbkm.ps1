...
# ~<> Docker's Zimit Automated Script v1.0 <>~ #
...
# Append commands to text file in current directory then runs them
    ...
    $dockerCMD = "run -v $wFolder\$itemname2`:/crawls docker.io/webrecorder/browsertrix-crawler:latest crawl --name $itemname2 --url $item --scopeType $scopeType1 --depth -1 --extrahops 1 --timeout 10000 --waitUntil load,networkidle2 --postLoadDelay 2 --pageExtraDelay 1 --workers 16 --exclude=`"(action=edit|youtube.com|silver_surfer.html|index.php\?title|redirect=)`" --keep --verbose --continue-on-error"
